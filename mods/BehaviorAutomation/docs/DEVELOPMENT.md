...
脚本只构建与打包，输出到 `.artifacts/BehaviorAutomation/package/`。ZIP 内只有 DLL、manifest、两份语言文件、使用说明和 LICENSE，不会安装到游戏 Mods。
...
| `WorldTargets.cs` | 目标分类与待处理条件 |
| `WorkController.cs`、`ToolExecution.cs` | 调度和原生动作执行 |
| `Navigation.cs`、`WalkRoute.cs`、`ObstaclePlanner.cs` | 寻路、移动与受配置限制的清障 |
...
结果位于同次运行的 `evidence/native-result.json` 与 `native-tests.txt`，包含被测 DLL 哈希、通过、失败和跳过数。编译成功或 `-PrepareOnly` 完成不代表原生回归通过。
